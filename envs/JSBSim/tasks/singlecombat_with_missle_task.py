...
from ..reward_functions import RelativeAltitudeReward,RelativePositionReward,StableApproachReward,EscapeMissileReward
from ..core.simulatior import MissileSimulator
from ..utils.utils import LLA2NEU, get_AO_TA_R, get_thetaT_thetaA_R,get_forward_vector_from_rpy
...
class SingleCombatDodgeMissileTask(SingleCombatTask):
    ...
    def __init__(self, config):
        ...
        # ]
        self.reward_functions = [
            RelativePositionReward(self.config)
            # AltitudeReward(self.config),
            # EventDrivenReward(self.config)
        ]
    ...
    def get_obs(self, env, agent_id):
        ...
        # (0) extract feature: [north(km), east(km), up(km), v_n(mh), v_e(mh), v_d(mh)]
        ...
        norm_obs[1] = np.sin(ego_obs_list[3])#滚转角sin值
        norm_obs[2] = np.cos(ego_obs_list[3])#滚转角cos值
        norm_obs[3] = np.sin(ego_obs_list[4])#俯仰角sin值
        norm_obs[4] = np.cos(ego_obs_list[4])#俯仰角cos值
        norm_obs[5] = ego_obs_list[9] / 340#速度
        ...
        ego_feature_rpy = np.concatenate((ego_feature, ego_obs_list[3:6]))
        enm_feature_rpy = np.concatenate((enm_feature, enm_obs_list[3:6]))
        thetaT,thetaA,_=get_thetaT_thetaA_R(ego_feature_rpy, enm_feature_rpy)
        norm_obs[9] = (enm_obs_list[9] - ego_obs_list[9]) / 340#速度差
        #改成三个坐标的速度差
        norm_obs[10] = (enm_obs_list[2] - ego_obs_list[2]) / 1000#高度差
        norm_obs[11] = ego_AO/np.pi
        norm_obs[12] = ego_TA/np.pi
        norm_obs[13] = R / 10000
        norm_obs[14] = side_flag
        norm_obs[15] = thetaT / np.pi
        norm_obs[16] = thetaA / np.pi
        norm_obs[17] = np.sin(enm_obs_list[3])# enemy roll
        norm_obs[18] = np.cos(enm_obs_list[3])# enemy roll
        norm_obs[19] = np.sin(enm_obs_list[4])  # enemy pitch
        norm_obs[20] = np.cos(enm_obs_list[4])  # enemy pitch
        norm_obs[21] = np.sin(enm_obs_list[5])  # enemy yaw
        norm_obs[22] = np.cos(enm_obs_list[5])  # enemy yaw
        ...
        if missile_sim is not None:
            ...
            norm_obs[23] = (np.linalg.norm(missile_sim.get_velocity()) - ego_obs_list[9]) / 340
            norm_obs[24] = (missile_feature[2] - ego_obs_list[2]) / 1000
            norm_obs[25] = ego_AO
            norm_obs[26] = ego_TA
            norm_obs[27] = R / 10000
            norm_obs[28] = side_flag
        return norm_obs
    ...
    def step(self, env):
        ...
        for agent_id, agent in env.agents.items():
            ...
            posture = agent.get_rpy()# Get the heading direction of the agent
            heading=get_forward_vector_from_rpy(posture)#方向使用roll, pitch, yaw变换成方向矢量
            distance = np.linalg.norm(target)
            attack_angle = np.rad2deg(np.arccos(np.clip(np.sum(target * heading) / (distance * np.linalg.norm(heading) + 1e-8), -1, 1)))#两个向量之间的夹角
...
class SingleCombatShootMissileTask(SingleCombatDodgeMissileTask):
    def __init__(self, config):
        ...
        self.reward_functions = [
            # RelativePositionReward(self.config),
            # AltitudeReward(self.config),
            # StableApproachReward(self.config),
            # EscapeMissileReward(self.config),
            # RelativePositionReward(self.config)
            # EventDrivenReward(self.config)
            # EscapeMissileReward(self.config),
            RelativePositionReward(self.config)
